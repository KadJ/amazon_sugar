...
To configure additional system settings, click <a href="index.php?module=Administration&action=index" target="_blank">here</a>.',
  ...
  'LBL_ASTERISK_OPENCNAM_USERNAME' => 'OpenCNAM Username (Paid API)',
  'LBL_ASTERISK_OPENCNAM_USERNAME_DESC' => 'Username is only required if you want to use the OpenCNAM paid API.  Leave blank otherwise.',
  'LBL_ASTERISK_OPENCNAM_RETRIES' => 'OpenCNAM Retry Attempts',
  'LBL_ASTERISK_OPENCNAM_RETRIES_DESC' => 'When a phone number is not in the OpenCNAM database it can take up to 20 seconds for the initial lookup.  OpenCNAM returns immediately with a 404 code if it is not already cached in it the database.  This makes callListner block for about 1000ms * number of retry attempts in an attempt to try and wait for the data to come back in a reasonable time frame. As a result, the more retry attempts the longer it will take for the first call popup to appear on a users screen.  Recommended this is not raised higher then 4.',
  'LBL_ASTERISK_OPENCNAM_APIKEY' => 'OpenCNAM Integration API Key (Paid API)',
  'LBL_ASTERISK_OPENCNAM_APIKEY_DESC' => 'An API Key is not required.  If you leave it blank, it\'ll use the free api which has a limitation of 60 lookups per hour.  The free API responses are slower.  Its fairly common for this to fail the first time someone calls.',
