...
EOQ;
	...
	if(!empty($_REQUEST['document'])){
      $countDocument = count($_REQUEST['document']);
    }
    else{
        $countDocument=10;
    }
	...
	///////////////////////////////////////////////////////////////////////////
        ...
        for($i=0; $i<$countPDF; $i++) {
            if(isset($_REQUEST['pdfId'.$i]) && !empty($_REQUEST['pdfId'.$i])) {
                $pdf = new AOS_PDF_Templates();

                $pdf->retrieve($_REQUEST['pdfId'.$i]);

                array_push($focus->saved_attachments, $pdf);
            }
