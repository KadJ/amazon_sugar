...
if(true) {
    ...
    // Прикрепленный PDF-Шаблон
    $seedPDFTemplate = new AOS_PDF_Templates();
    $templates = $seedPDFTemplate->get_full_list("", "`{$seedPDFTemplate->table_name}`.`type` IN ('Contacts')");
    $templates_smarty = array('' => '');
    if(count($templates)) {
        foreach($templates as $seedPDFTemplate) {
            $templates_smarty[$seedPDFTemplate->id] = $seedPDFTemplate->name;
        }
    }
    $xtpl->assign("PDFTEMPLATESOPTIONS", get_select_options_with_id($templates_smarty, $focus->pdf_template_id));
