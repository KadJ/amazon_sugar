...
function setPDFToCampaignTemplate(target, pdfId, pdfName, pdfType) {
    var pdfIdInput = window.opener.document.getElementById('pdfId' + target);
    var pdfNameInput = window.opener.document.getElementById('pdfName' + target);
    var pdfTypeInput = window.opener.document.getElementById('pdfType' + target);

    var pdfIdInput = window.opener.document.getElementById('pdfId');
    var pdfNameInput = window.opener.document.getElementById('pdfName');
    var pdfTypeInput = window.opener.document.getElementById('pdfType');

    pdfIdInput.value = pdfId;
    //docId.onchange('docUpload(); form_reset_doc();');
    pdfNameInput.value = pdfName;
    pdfNameInput.onchange('pdfUpload(); form_reset_doc();');
    //alert(docName.onchange);
    window.close();
}
...
function pdfUpload() {
    ...
    var url ='index.php?module=EmailTemplates&action=AttachPDF';
    YAHOO.util.Connect.asyncRequest('POST', url, callback,null);
    //AJAX call ends

    var element = document.getElementById('pdfName');
    var element1 = document.getElementById('pdfId');
    var element3 = document.getElementById('pdfType');
    var elm = document.createElement('div');
    elm.setAttribute('id','file'+uploadIndex);

    var sugarPDF = document.createElement('input');
    sugarPDF.setAttribute('type', 'label');
    sugarPDF.setAttribute('disabled', 'true');
    sugarPDF.setAttribute('font', 'bold');
    sugarPDF.setAttribute('value',"Sugar PDF");
    ...
    var new_row_chk_box = document.createElement( 'input' );
    new_row_chk_box.setAttribute('id','checkBoxPDF'+uploadIndex);
    new_row_chk_box.setAttribute('name','checkBoxPDF'+uploadIndex);
    new_row_chk_box.type = 'checkbox';
    new_row_chk_box.checked =false;
    new_row_chk_box.disabled='true';
    ...
    var eah = document.createElement('input');
    eah.setAttribute('id', 'pdfId'+uploadIndex);
    eah.setAttribute('name', 'pdfId'+uploadIndex);
    eah.setAttribute('tabindex', '120');
    eah.setAttribute('type', 'hidden');
    eah.setAttribute('value',element1.value);
    ...
    var attType = document.createElement('input');
    attType.setAttribute('id', 'pdfType'+uploadIndex);
    attType.setAttribute('name', 'pdfType'+uploadIndex);
    attType.setAttribute('tabindex', '120');
    attType.setAttribute('type', 'hidden');
    attType.setAttribute('value',element3.value);
    ...
    var ea = document.createElement('input');
    ea.setAttribute('id', 'pdf[]');
    ea.setAttribute('name', 'pdf[]');
    ea.setAttribute('tabindex', '120');
    ea.setAttribute('size', '40');
    ea.setAttribute('type', 'text');
    ea.setAttribute('disabled',true);
    ...
    new_row_button_embed.onclick= function(){
        ...
        if(mime_type == "image/gif" || mime_type == "image/bmp" || mime_type == "image/png" || mime_type == "image/x-png" || mime_type == "image/jpg" || mime_type == "image/jpeg")
        {
            embedImage='<img src="index.php?entryPoint=download&type=PDF&id='+documentRevisionId+'">';
            insert_variable(embedImage);
        }
        else{
            new_row_chk_box.checked =false;
    ...
    var SugarPDF = document.createElement("img");
    SugarPDF.setAttribute("src", "index.php?entryPoint=getImage&themeName="+SUGAR.themes.theme_name+"&imageName=sugar_document.png");
    SugarPDF.setAttribute("align","absmiddle");
    SugarPDF.setAttribute("alt",lbl_email_attachments_document);
    SugarPDF.setAttribute("border","0");
    SugarPDF.setAttribute("height","16");
    SugarPDF.setAttribute("width","16");


    //elm.appendChild(eah);
    elm.appendChild(SugarPDF);
    elm.appendChild(new_row_button_embed);
    elm.appendChild(new_row_chk_box);
    elm.appendChild(eah);
    elm.appendChild(attType);
...
function selectPDF() {
    URL="index.php?module=EmailTemplates&action=PopupPDFCampaignTemplate&to_pdf=true&target=" ;
    windowName = 'selectPDF';
    windowFeatures = 'width=800' + ',height=600' + ',resizable=1,scrollbars=1';
