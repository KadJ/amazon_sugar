...
class AOS_PDF_TemplatesViewEdit extends ViewEdit {
	...
	function setFields(){
		...
		foreach($modules as $moduleName => $value) {
			...
			$options = get_select_options($options_array,'');
		...
		$options = get_select_options($options_array,'');
			...
			$module_options = get_select_options($mod_options_array,'');
			$module_options = preg_replace( "'\n'", '', $module_options);
		...
		if($this->bean->type=='Quotes') {
			...
			$mod_options = $moduleOptions[$modules[0]]['module'];
			$var_options = $moduleOptions[$modules[0]]['option'];
		}else{
			$mod_options = $moduleOptions[$this->bean->type]['module'];
			$var_options = $moduleOptions[$this->bean->type]['option'];
		}
