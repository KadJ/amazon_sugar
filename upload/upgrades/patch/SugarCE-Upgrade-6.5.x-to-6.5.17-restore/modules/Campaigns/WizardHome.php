...
global $currentModule;
    ...
    if($focus->campaign_type =='NewsLetter' || $focus->campaign_type =='Email'){
        // Если это емайл-рассылка
        $ss->assign("MARKETING_TBL", create_marketing_summary ($focus));
    }

    if($focus->campaign_type == 'Fax'){
        // Если это рассылка факсов
        $ss->assign("MARKETING_FAX_TBL", create_marketing_fax_summary ($focus));
    }
...
function create_marketing_fax_summary  ($focus){
    global $mod_strings,$app_strings;

    $smarty = new Sugar_Smarty();

    // Шаблоны PDF
    $pdf_templates_smarty = array();
    $seedPDFTemplate = new AOS_PDF_Templates();
    $pdf_templates = $seedPDFTemplate->get_full_list();

    foreach($pdf_templates as $seedPDFTemplate) {
        $pdf_templates_smarty[$seedPDFTemplate->id] = $seedPDFTemplate->name;
    }

    // Данные по маркетинговой компании
    $campaing_smarty = array(
        "ID" => $focus->id,
    );



    $smarty->assign("campaing", $campaing_smarty);
    $smarty->assign("PDF_TEMPATE_OPTIONS", get_select_options_with_id($pdf_templates_smarty, $focus->aos_pdf_templates_id_c));

    return $smarty->fetch("modules/Campaigns/tpls/faxTable.tpl");
}
