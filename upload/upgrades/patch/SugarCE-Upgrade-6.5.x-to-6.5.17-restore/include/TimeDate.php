...
/****************** GUI stuff that really shouldn't be here, will be moved ************/
    ...
    /**
     * Получить название месяца для даты
     * @param $timedate
     * @return mixed
     */
    function getMonthString($timedate, $type = 1) {
        $month_array_1 = array(
            '01' => 'январь',
            '02' => 'февраль',
            '03' => 'март',
        ...
        switch($type) {
            case '1':
                return $month_array_1[date('m', $timedate)];
                break;
            case '2':
                return $month_array_2[date('m', $timedate)];
                break;
        }
    ...
    /**
     * Получить количество дней, сколько надо прибавить к указанному, чтобы вышло прибавить нужное кол-во рабочих дней
     * @param $days
     * @param bool $timestamp
     * @return int
     */
    function getAddWorkDays($days, $timestamp = false) {
        if(!$timestamp) {
            $timestamp = time();
        }
        $num_work_days = 0;
        for($i = 0; $i <= $days; $i++) {
            $current_day = (int)date("N", strtotime("+" . $num_work_days . " day", $timestamp));
            if($current_day == '6') {
                // Если день приходится на субботу
                $num_work_days += 2;
            } elseif($current_day == '7') {
                // Если день приходится на воскресенье
                $num_work_days += 2;
            } else {
                // Рабочий день
                $num_work_days++;
